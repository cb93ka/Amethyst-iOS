...
int launchJVM(NSString *username, id launchTarget, int width, int height, int minVersion) {
    ...
    if (requiresTXMWorkaround) {
        ...
        if ((uint32_t)result != 0x690000E0) {
            ...
            if(lcAppInfo) {
                // if this is inside LiveContainer, we assign script ourselves and prompt user to restart Aurora
                ...
                if([lcAppInfo writeToFile:lcAppInfoPath atomically:YES]) {
                    showDialog(localize(@"Error", nil), @"Aurora was launched with a legacy script. We have updated the script to Universal, please restart LiveContainer to continue.");
            ...
            showDialog(localize(@"Error", nil), @"Support for legacy script has been removed. Please switch to Universal JIT script. To import it, long-press on Aurora when enabling JIT in StikDebug and tap \"Assign Script\", then go to Aurora's Documents directory and pick it. (on sideloaded StikDebug, the builtin script is named Amethyst-MeloNX.js)");
            [PLLogOutputView handleExitCode:1];
            return 1;
        }
